...
def create_document(title, source_text, source_lang="en"):
    conn = get_conn()
    now = _now()
    cursor = conn.execute(
        "INSERT INTO documents (title, source_text, source_lang, current_stage, status, created_at, updated_at) VALUES (?, ?, ?, 1, 'in_progress', ?, ?)",
        (title, source_text, source_lang, now, now),
    )
